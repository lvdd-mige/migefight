...
##### U^2-Net ####
class U2NET_Test(nn.Module):

    def __init__(self,in_ch=3,out_ch=1):
        super(U2NET_Test,self).__init__()
    ...
    def forward(self,x):
        ...
        return F.sigmoid(d0),F.sigmoid(d1)
...
if __name__ == '__main__':
    test = torch.Tensor(np.random.randint(0, 10, size=[1, 1,  512, 512])).cuda()
    print(test.shape, test.type)
    net = U2NET(1, 1).cuda()
